...
export function RejectSampleModal({
  ...
  // Multi-style apply scope. This used to default to 'all_on_po', which meant
  // opening the reject modal and pressing Reject rejected every sibling style
  // on the PO — while Approve defaulted to just the one you clicked. The
  // destructive action shouldn't be the one that fans out by default, so it
  // now starts narrow like every other scope picker (see DEFAULT_SCOPE).
  ...
  const effectiveCount = useMemo(
    () => scopeTargetCount(applyScope, siblings.length, selectedSiblingOrderIds.size),
      ...
      >
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto">
          ...
          <ScopePicker
            ...
            selectedOrderIds={selectedSiblingOrderIds}
