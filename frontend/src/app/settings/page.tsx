...
type EmailAutomation = {
  ...
  setting_key: string;
  enabled: boolean;
  /** False when the automation picks its own recipients (e.g. @mentions). */
...
function AutomationRow({
  ...
  // Some automations work out their own audience — a mention emails the person
  // who was mentioned. For those there is no list to show, and the old UI
  // offered a picker plus a "no one will get emails" warning that was simply
  // wrong: mentions were sending the whole time. Toggle only.
  const usesRecipients = automation.uses_recipients !== false;
  const showNoRecipientsWarning = usesRecipients && automation.enabled && recipients.length === 0;

  return (
    <div className="px-5 py-3">
      <div className="flex items-center justify-between gap-4">
        <button
          onClick={() => usesRecipients && setExpanded((v) => !v)}
          disabled={!usesRecipients}
          className={cn(
            'flex items-start gap-2 flex-1 min-w-0 text-left group',
            !usesRecipients && 'cursor-default',
          )}
        >
          <ChevronRight
            className={cn('w-3.5 h-3.5 text-gray-400 mt-0.5 flex-shrink-0 transition-transform group-hover:text-gray-600',
              expanded && 'rotate-90', !usesRecipients && 'invisible')}
          />
...
function NotificationsTab() {
  ...
  const toggleEmails = async () => {
    ...
    setSaving(true);
    try {
      await settingsApi.updateAppSettings({ emails_enabled: next });
      toast.success(next ? 'Automatic emails enabled' : 'Automatic emails disabled');
    } catch {
      setEmailsEnabled(!next);
