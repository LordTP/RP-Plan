...
/** An unsaved shipment — exists only in this component's state until the
 *  user presses Save. Shaped like a real one so every downstream piece
 *  (checklist, picker, confirm preview) works without a special case. */
function blankDraft(factory: string): ShipmentDraftDetail {
...
function DraftDetail() {
  ...
  // "new" is a shipment that does not exist yet. It lives entirely in state
  // until the first Save, which is what creates the record.
  const routeId = String(params?.draftId ?? '');
  const isNew = routeId === 'new';
  const draftId = isNew ? 0 : Number(routeId);
  const newFactory = isNew
    ? (searchParams?.get('factory') || user?.factory_name || '')
    : '';
  ...
  const loadAll = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // A new shipment has nothing to fetch — only the picker, so the user
      // has styles to choose from.
      const d = isNew ? blankDraft(newFactory) : await shipmentDraftsApi.get(draftId);
      setDraft(d);
  ...
  // Which individual fields differ from the saved version — the input picks
  // up a blue edge, so an unsaved edit is visible at the field, not just in
  // the footer.
  const changedFields = useMemo(() => {
    const c = new Set<string>();
    if (!draft) return c;
    if (reference !== draft.reference) c.add('reference');
    if ((name || '') !== (draft.name || '')) c.add('name');
    if ((fclLcl || '') !== (draft.fcl_lcl || '')) c.add('fcl_lcl');
    if ((vesselName || '') !== (draft.vessel_name || '')) c.add('vessel_name');
    if (vesselEtd !== (draft.vessel_etd ? draft.vessel_etd.slice(0, 10) : '')) c.add('vessel_etd');
    if (vesselEtaToPort !== (draft.vessel_eta_to_port ? draft.vessel_eta_to_port.slice(0, 10) : '')) c.add('vessel_eta_to_port');
    if ((trackingReference || '') !== (draft.tracking_reference || '')) c.add('tracking_reference');
    return c;
  }, [draft, reference, name, fclLcl, vesselName, vesselEtd, vesselEtaToPort, trackingReference]);
  ...
  /** Writes everything staged — creating the shipment first if this is a new
   *  one. Returns the shipment id, or null if anything failed, so Confirm
   *  knows whether to carry on. Does not navigate; callers decide that. */
  const persist = async (): Promise<number | null> => {
    ...
    try {
      if (isNew) {
        if (!newFactory) {
          toast.error('No factory set for this shipment — start again from the shipments list');
          return null;
        }
        // One call creates the shipment with its styles already on it. Links
        // default to the full PO quantity, so only push the ones the user
        // actually changed.
        const created = await shipmentDraftsApi.create({
          factory: newFactory,
          reference: reference.trim() || null,
          order_ids: Array.from(selection.keys()),
          ...fields,
        });
        for (const [orderId, qty] of Array.from(selection.entries())) {
          if (qty !== (totalQtyByOrderId.get(orderId) || 0)) {
            await shipmentDraftsApi.updateOrderQuantity(created.id, orderId, qty);
          }
        }
        toast.success(`Shipment ${created.reference} created`);
        return created.id;
      }

      // Removals first: the link table is unique per (draft, order), so
      // removing before adding keeps a swap clean.
      ...
      await shipmentDraftsApi.update(draft.id, { reference, ...fields });
      toast.success('Saved');
      await refresh();
      return draft.id;
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || 'Failed to save');
      // Part of it may have gone through before the failure, so pull the
      // server state back rather than leaving a diff that no longer exists.
      await refresh();
      return null;
    } finally {
      setIsSaving(false);
  ...
  const handleSave = async () => {
    ...
    // A new shipment now has a real id — swap the URL for it so a refresh,
    // a back button or a second Save all land on the saved record.
    if (id && isNew) router.replace(`/factory-shipping/${id}`);
  };
  ...
  const handleConfirm = async () => {
    ...
    let id: number | null = draft.id;
    if (isNew || isDirty) {
      id = await persist();
      if (!id) { setShowConfirmModal(false); return; }
    }
    setIsConfirming(true);
    try {
      await shipmentDraftsApi.confirm(id);
        ...
        <div className="flex-1 min-h-0 bg-white rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] ring-1 ring-gray-100 overflow-hidden flex flex-col">
          ...
          <div className={cn(
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              {isNew && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide bg-amber-100 text-amber-800 border border-amber-200 flex-shrink-0">
                  Not created yet
                </span>
              )}
              <div className="min-w-0 flex-1">
                <input
                  ...
                  disabled={skusLocked}
                  placeholder={isNew ? 'P-number, or leave blank' : ''}
                  title="Most factories name a shipment after its P-number"
                  className={cn(
                    'text-base font-bold text-gray-900 bg-transparent border border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500/20 rounded px-1 -ml-1 w-full max-w-[280px] disabled:opacity-100 disabled:text-gray-900 placeholder:font-medium placeholder:text-gray-400',
                    changedFields.has('reference') && 'border-blue-300 bg-blue-50/50'
                  )}
                />
                <input
                  type="text"
                  ...
                  className={cn(
                    'block text-xs text-gray-500 bg-transparent border border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500/20 rounded px-1 -ml-1 w-full max-w-[280px] placeholder:text-gray-400 disabled:opacity-100',
                    changedFields.has('name') && 'border-blue-300 bg-blue-50/50'
                  )}
                />
              </div>
            </div>
          ...
          {/* Save bar — spans the whole card so it covers both columns.
              Every edit above is staged until this is pressed. */}
          {!isLocked && !editingConfirmed && (
            <div className={cn(
              'px-6 py-2.5 border-t flex items-center gap-3 flex-shrink-0',
                ...
                {isDirty ? (
                  <>
                    <div className="text-[11.5px] font-bold text-amber-800 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                      {isNew ? 'Not created yet' : 'Not saved yet'}
                    </div>
                    <div className="text-[10.5px] text-amber-700 truncate">{pendingSummary}</div>
                  </>
                ) : (
                  <div className="text-[11.5px] text-gray-500">
                    {isNew
                      ? 'Nothing added yet. Pick the styles going on this shipment, fill in the vessel details, then save.'
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold border border-gray-300 bg-white text-gray-700 rounded-md hover:bg-gray-50 disabled:opacity-40 flex items-center gap-1.5"
              >
              ...
              <button
                onClick={handleSave}
                disabled={isSaving || !isDirty}
                className="px-3 py-1.5 text-xs font-semibold bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-40 flex items-center gap-1.5"
              >
                {isSaving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                {isNew ? 'Create shipment' : isDirty ? 'Save' : 'Saved'}
              </button>
            </div>
          )}
        </div>
